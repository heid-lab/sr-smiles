...
def load_test_reactions(dataset: str, include_all_test_data: bool) -> list[str]:
    """Load only real reaction data from tests/data."""
    data_dir = PROJECT_ROOT / "tests" / "data"

    if dataset == "sr_test_cases":
        reactions = _extract_reactions_from_csv(data_dir / "sr_test_cases.csv")
    else:
        csv_path = Path(dataset)
        if not csv_path.is_absolute():
            csv_path = PROJECT_ROOT / dataset
        reactions = _extract_reactions_from_csv(csv_path)

    if include_all_test_data:
        for extra_csv in sorted(data_dir.rglob("*.csv")):
            reactions.extend(_extract_reactions_from_csv(extra_csv))
...
def _build_transform(
    api: str, keep_atom_mapping: bool, remove_hydrogens: bool, balance_rxn: bool
) -> Callable[[str], str]:
...
def run_diagnostics(args: argparse.Namespace) -> dict:
    ...
    reactions = load_test_reactions(dataset=args.dataset, include_all_test_data=args.include_all_test_data)

    if not reactions:
        raise ValueError("No reactions found in selected test data source.")

    if args.max_samples and args.max_samples > 0:
        reactions = reactions[: args.max_samples]

    if args.only_balanced:
        reactions = [r for r in reactions if is_balanced(r)]
    ...
    print(f"Loaded {len(reactions)} reactions from test data.")
    print(f"API={args.api}, repeats={args.repeats}, warmup={args.warmup}")

    for _ in range(args.warmup):
        _run_once(transform, reactions)

    run_seconds: list[float] = []
    for _ in range(args.repeats):
        t0 = time.perf_counter()
        output = _run_once(transform, reactions)
        run_seconds.append(time.perf_counter() - t0)

    tracemalloc.start()
    _ = _run_once(transform, reactions)
    _, peak_bytes = tracemalloc.get_traced_memory()
    tracemalloc.stop()

    total_rxns = len(reactions)
    ...
    results = {
        ...
        "throughput_rxn_per_sec_avg": total_rxns / avg_sec,
        "latency_ms_per_rxn_avg": (avg_sec / total_rxns) * 1000,
    ...
    if args.profile_cpu:
        ...
        profile.enable()
        _run_once(transform, reactions[: args.profile_samples])
        profile.disable()
    ...
    if args.profile_lines:
        try:
            ...
            lp.runcall(lambda: _run_once(transform, reactions[: args.profile_samples]))
    ...
    print(f"Throughput(avg): {results['throughput_rxn_per_sec_avg']:.2f} rxn/s")
